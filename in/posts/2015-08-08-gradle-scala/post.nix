...

{
  ...
  abstract = ''I recently switched a Scala project’s build from SBT to Gradle
    and was disappointed to find that Gradle has no support for launching the
    Scala REPL. My workaround: …'';
  ──────────────────────────────────────────────────────────────────────────────
  ...
  body =
    ...
    in [
      ...
      (p ''I recently switched a Scala project’s build from SBT to Gradle and
        was disappointed to find that Gradle has no support for launching the
        Scala REPL.'')
      ...
      (p ''At first this felt like a kludge around a deficiency in Gradle, but
        now I think this approach may actually make more sense than SBT’s
        conflation of build tool and REPL. The build tool doesn’t need to be
      ...
      (p ''To let Gradle provide the scala-compiler artifact for us, add it as a
        `classpath` dependency to `buildscript`.'')
      ...
      (p ''Then add a function to look up the filesystem path of this artifact,
        which we’ll use later when assembling the full classpath for the REPL
        session.'')
      ...
      (p ''Getting the classpath for your project and its dependencies in Gradle
        is pretty simple.'')
      ...
      (p ''Combined with the path of the Scala compiler, the result is the full
      ...
      (p ''With this task defined, call `gradle printClasspath --quiet` to set
        the classpath in the startup script.'')
      ...
      (p ''SBT has a useful setting that lets you specify Scala expressions that
      ...
      (p ''You can accomplish this using the `-i` option on the Scala REPL,
        which loads commands from a file. In this example, the file containing
        initial commands is named `repl.scala`.'')
